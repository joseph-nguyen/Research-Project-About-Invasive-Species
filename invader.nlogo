...
[
  ...
  invaders-energy-gain-from-bugs ;; how much energy birds gain from eating invaders
...
to setup
  ...
  set birds-energy-gain-from-bugs 35
  set invaders-energy-gain-from-bugs 10
  ...
  set-default-shape invaders "hog"
  add-starting-grass
  add-bugs
  add-birds
  invader-move
...
to invader-move
  ...
  if weather?
  [set invaders-stride 0.01 * %-invader-move-in-weather]
end
...
to remove-a-%-invaders
  let number-invaders count invaders
  ask n-of floor (number-invaders * %-of-invaders-kill-by-hunt / 100) invaders [
    hatch-disease-markers 1 [
     set current-age  0
     set size 3
     set color red
...
to disease-a-%-invaders
  let number-invaders count invaders
  ask n-of floor (number-invaders * %-invader-die-by-disease / 100) invaders [
    if random 100 <  %-invader-die-by-disease [
    die]
  ]
end
...
to age-disease-markers
  ...
  if current-age  > 10  or (ticks = 999 and constant-simulation-length?)  [die]
end
...
to trap-eat-invader  ;; birds procedure
    if (any? invaders-here) [
...
to invaders-eat-prey  ;; birds procedure
    if (any? bugs-here)
        [ask one-of  bugs-here
          [die]
           set energy (energy + invaders-energy-gain-from-bugs)
         ]
end
...
Birds and bugs wander randomly around the landscape.  Each step costs both animals energy and they must consume a food source (bugs must eat grass and birds must eat bugs ) to replenish their energy - when they run out of energy they die. To allow the population to continue, each bird or bug must have enough energy to have a litter of offspring and the offspring and parent split the energy amongst themselves.  Grass grows at a fixed rate, and when it is eaten, a fixed amount of grass energy is deducted from the patch (square) where the grass was eaten.

Invasive species (Hog) can be introduced to this ecosystem as well. The hog are indirect competitors of the bugs. They eat grass & bug.  They reproduce following the same rules as bugs.  The eating behavior of the mice and the bugs can be adjusted.

Other disruptions such as removing a portion of the bug and hog population (simulating a disease, control number of trap on/hunting, change the weather) or burning down the grass (removing the food source for bugs and invasive species temporarily) can also be tested to see how these disruptions affect the stability of the ecosystem.
...
MIN-REPRODUCE-ENERGY_INVADERS The amount of energy required for a hog to accumulate before it automatically produces an offspring.
AMOUNT-OF-FOOD-INVADERS-EAT: The amount of grass consumed at each simulation step by each one of the invasive species.
%-OF-INVADERS-KILL-BY-HUNT: Sets the % of hogs that will have all their hogs immediately removed when the INVADER HUNTING button is pressed.
%-INVADER-MOVE-IN-WEATHER: Sets the % of weather that levels from favorable to harsh will apply all the hogs when the WEATHER switch is on.
NUMBER-OF-TRAP: Sets the number of trap that will be random set on when the TRAP button is pressed.
%-INVADER-DIE-BY-DISEASE: Sets the % of existing hog population that will be immediately removed when the INVADER DISEASE button is pressed.
...
Phuc Nguyen Nguyen
Mira Moon
CDS-201-001 (Fall 2020)
Intruction by Prof. Dale S. Rothman
Final Project


REFERENCES

This model is modified from the Bug Hunt Predators and Invasive Species
...
false
0
Polygon -7500403 true true 180 90 180 90 210 105 225 120 255 105 240 135 255 150 255 165 270 180 285 180 270 210 240 210 210 195 195 210 195 255 180 255 180 210 90 210 90 255 75 255 75 210 45 165 45 120 30 105 30 90 60 105 105 90 150 90 180 90 210 105
